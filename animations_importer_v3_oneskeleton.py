...
destination_path = r'/Game/LAST_TEST_OS'  # folder in UE to import
models_path = r'/Game/Meshes/OneSkeleton'  # folder in UE with your models
sequence_name = '02'  # your sequence name

player_skeleton = r'/Game/Meshes/OneSkeleton/players/Player_Skeleton'
glove_skeleton = r'/Game/Meshes/OneSkeleton/hands/agr_hands_Skeleton'

import_cameras = True  # import_cameras?
import_players = True  # import_players?
import_pov = True  # import_pov?
import_weapons = True  # import_world_weapons?
import_visibility = True  # import visibility from generated JSON file

save_current_level = True  # save_current_level_before_import?
clean_names = True  # Fix ' mdl' at the end of the animation's name?
cleanup = True  # This will create 3 folders (pov, weapons and players) and distribute animations between them
...
def agr_importer(prefix, foldername):
    ...
    def fbx_importer(os_file, skeleton, obj):
        log('Importing .fbx file: {}'.format(os_animation_fullname))
    ...
    def ue_mesh_placer(animation_path):
        ...
        mesh_component.set_bounds_scale(float(bounds_scale))
        if receives_decals:
        ...
        if prefix is 'tm_':
            ...
            p_pattern = 'afx.(\\d|\\d+) (.+)(.mdl.fbx)'
            p_replacement = '\\2'
            if skeleton_finder(p_pattern, p_replacement) is not None:
                mesh_component.set_skeletal_mesh(skeleton_finder(p_pattern, p_replacement)[1])
            else:
                log_e('MESH FOR {} WAS NOT FOUND'.format(os_animation_fullname))
        elif 'glove' in os_animation_fullname and prefix in os_animation_fullname:
            if 'hardknuckle' in os_animation_fullname:
                p_pattern = 'afx.(\\d|\\d+) (v_glove_hardknuckle).+'
                p_replacement = 'agr_\\2'
                if skeleton_finder(p_pattern, p_replacement) is not None:
                    mesh_component.set_skeletal_mesh(skeleton_finder(p_pattern, p_replacement)[1])
                else:
                    log_e('MESH FOR {} WAS NOT FOUND'.format(os_animation_fullname))
    ...
    for dirpath, dirs, files in os.walk(agr_path):
        for filename in files:
            if filename.endswith('.mdl.fbx'):
                global os_animation_fullname
                os_animation_fullname = os.path.join(dirpath, filename)

                if slow_task.should_cancel():
                    break

                def g_p_skeleton(skeleton):
                    if skeleton_finder_custom(skeleton) is not None:
                        main_importer(skeleton_finder_custom(skeleton))
                    else:
                        log_e('SKELETON FOR {} WAS NOT FOUND'.format(os_animation_fullname))

                if prefix in os_animation_fullname and 'glove' not in os_animation_fullname and 'sleeve' not in os_animation_fullname and 'tm_' not in os_animation_fullname:
                    pattern = 'afx.(\\d|\\d+) (.+)(.mdl.fbx)'
                    replacement = '\\2_Skeleton'
                    if skeleton_finder(pattern, replacement) is not None:
                        main_importer(skeleton_finder(pattern, replacement))
                    else:
                        log_e('SKELETON FOR {} WAS NOT FOUND'.format(os_animation_fullname))

                elif 'glove' in os_animation_fullname and prefix in os_animation_fullname:
                    g_p_skeleton(glove_skeleton)

                elif 'sleeve' in os_animation_fullname and prefix in os_animation_fullname:
                    g_p_skeleton(glove_skeleton)

                elif 'tm_' in os_animation_fullname and prefix in os_animation_fullname:
                    g_p_skeleton(player_skeleton)
